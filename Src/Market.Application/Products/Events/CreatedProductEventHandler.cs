...
public class CreatedProductEventHandler : IEventHandler<CreatedProductDomainEvent>
{
    private readonly IProductEventStore productEventStore;
    private readonly IReposeCache reponseCache;

    public CreatedProductEventHandler(IProductEventStore productEventStore, IReposeCache reponseCache)
    {
        this.productEventStore = productEventStore;
        this.reponseCache = reponseCache;
    }

    public async Task Handle(CreatedProductDomainEvent @event, CancellationToken cancellationToken)
    {
