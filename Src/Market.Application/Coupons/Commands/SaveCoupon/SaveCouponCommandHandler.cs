...
public class UserSaveCouponCommandHandler : ICommandHandler<SaveCouponCommand, Guid>
{
    ...
    public async Task<Guid> Handle(SaveCouponCommand request, CancellationToken cancellationToken)
    {
        CouponId couponId = new(request.CouponId);
        UserId userId = new(request.UserId);

        var coupon = await couponRepository.GetCouponByIdAsync(couponId);
        coupon.SaveCoupon(userId);
